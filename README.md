...
- **Frontend**: Vite + React 18 + TypeScript + Material-UI + Google Maps + Pusher
- **Backend**: Node.js + Express + TypeScript + Prisma + PostgreSQL + Pusher
- **Auth**: JWT (HttpOnly Cookie) + Guest Token + bcrypt
- **Real-time**: Pusher (位置更新、到達通知、互動事件)
...
### Stage 7: Event Room - 即時聚會定位 🚧 進行中

全新的聚會活動功能，專注於「集合前後的即時定位追蹤」：

#### 核心功能
- [ ] **建立聚會活動**：設定時間、地點、時間窗（集合前後 30 分鐘）
- [ ] **Guest 模式加入**：無需註冊，填暱稱即可加入
- [ ] **即時定位分享**：只在時間窗內追蹤成員位置（使用 Pusher 即時推送）
- [ ] **到達標記**：「我到了」按鈕標記到達時間
- [ ] **戳人互動**：戳遲到的朋友（限制 3 次/人）
- [ ] **聚會結束排行榜**：金銀銅牌、遲到統計、被戳排名
- [ ] **Sidebar 導覽**：我的聚會、個人統計、排行榜

#### 技術亮點
- **Pusher 即時推送**：位置更新、到達通知、戳人互動
- **智能定位更新**：只在移動超過 50m 時更新，節省資源
- **Mobile-First 設計**：優先針對手機端優化
- **Guest Token 機制**：localStorage 儲存，無需註冊也能參與

#### 開發進度
- ✅ API 規格文件 (Issue #10)
- 🚧 Mock Data & TypeScript (Issue #11)
- 🚧 Events API 層 (Issue #12)
- ⏳ Pusher Hook (Issue #13)
- ⏳ EventRoom 頁面
- ⏳ 排行榜 Popup
- ⏳ Sidebar 元件

> 📝 **設計理念**：MeetHalf 功能做為「決定地點」的工具（選用），Events 功能專注於「集合過程」的即時追蹤和趣味互動。

---

### 🎯 專案狀態

**MeetHalf 核心功能** (Stage 0-6): ✅ 完成
- 完整的用戶認證系統
- 群組與成員管理
...
- Pusher Account（用於即時推送功能，請見下方說明）
...
### 2. 設定 Pusher（Stage 7 即時功能使用）

1. 前往 [Pusher Dashboard](https://dashboard.pusher.com/)
2. 註冊帳號並建立新的 Channels app
3. 選擇 **Cluster**（建議選 `ap3` - 亞太區）
4. 取得憑證：
   - **App ID**
   - **Key** (前端使用)
   - **Secret** (後端使用)
...
> 💡 **開發階段**：Pusher 免費方案提供每日 200K 訊息，足夠開發使用。
