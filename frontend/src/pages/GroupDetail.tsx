...
import { groupsApi, membersApi, offlineMembersApi, Group, TimeMidpointResponse, RoutesResponse, Member } from '../api/groups';
...
export default function GroupDetail() {
  ...
  // Debug logging
  console.log('Debug - user.id:', user?.id, 'group.ownerId:', group?.ownerId, 'isOwner:', isOwner);


  useEffect(() => {
    if (id) {
      fetchGroupData(true); // Auto-calculate on initial load
    }
  }, [id]);

  // Auto-refresh group data every 1 minute
  useEffect(() => {
    if (!id) return;

    const interval = setInterval(() => {
      fetchGroupData(false); // Don't auto-calculate midpoint on refresh
  ...
  const fetchGroupData = async (autoCalculateMidpoint = false) => {
    try {
      setLoading(true);
      setError(null);
      const groupResponse = await groupsApi.getGroup(Number(id));
      ...
      if (autoCalculateMidpoint) {
        const membersWithLocation = groupResponse.group.members.filter(m => m.lat && m.lng);
        if (membersWithLocation.length >= 2) {
          try {
            // Use time-based midpoint
            const timeMidpointResponse = await groupsApi.getTimeMidpoint(Number(id), {
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      // If user is not a member, add themselves to the group
      if (!currentUserMember) {
        await membersApi.addMember({
          groupId: group.id,
          userId: user.id,
        });
        setSnackbar({ open: true, message: '成功加入群組！', severity: 'success' });
      } else {
        // If user is already a member and wants to add someone else
        if (!newMemberEmail.trim()) {
          setSnackbar({ open: true, message: '請輸入成員的 Email 地址', severity: 'error' });
          return;
        }
        
        // In a real app, you would lookup user by email first
        // For now, show a message that this feature requires backend support
        setSnackbar({ 
          open: true, 
          message: '此功能需要後端支援通過 Email 查找用戶。目前請直接邀請用戶使用「加入群組」功能。', 
          severity: 'info' 
        });
        setNewMemberEmail('');
      }
      ...
      if (err.response?.status === 409) {
        errorMessage = '此用戶已經是群組成員！';
      } else if (err.response?.status === 404) {
        errorMessage = '找不到此用戶，請確認 Email 是否正確。';
  ...
  const handleCalculateTimeMidpoint = async () => {
    ...
    try {
      setCalculating(true);
      const response = await groupsApi.getTimeMidpoint(group.id, {
        objective: calcObjective,
      ...
      if (showRoutes) {
        try {
          ...
          const routesResponse = await groupsApi.getRoutesToMidpoint(group.id, {
  ...
  const handleToggleRoutes = async () => {
    if (!group || !timeMidpoint) return;

    if (!showRoutes) {
      // Fetch routes
      try {
        setLoadingRoutes(true);
        const response = await groupsApi.getRoutesToMidpoint(group.id, {
  ...
  const handleAddOfflineMember = async () => {
    if (!group || !offlineNickname.trim() || !offlineLocation) {
  ...
  const handleCopyGroupLink = async () => {
    ...
    try {
      ...
      setLinkCopied(true);
      setSnackbar({ open: true, message: '群組連結已複製到剪貼簿！', severity: 'success' });
  ...
  const handleUpdateGroupName = async () => {
    if (!group || !editedGroupName.trim()) return;
    
    try {
      setActionLoading(true);
      await groupsApi.updateGroup(group.id, { name: editedGroupName });
      setSnackbar({ open: true, message: '群組名稱已更新！', severity: 'success' });
  ...
  const markers = group ? group.members
    .filter((m) => m.lat && m.lng)
    .map((m) => ({
      id: m.id,
      lat: m.lat!,
      lng: m.lng!,
      title: m.isOffline ? `👤 ${m.nickname}` : m.user?.email || 'Unknown',
      label: m.isOffline 
        ? m.nickname || 'Friend' 
        : m.user?.email?.split('@')[0] || 'User',
  ...
  return (
    <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <Card sx={{ 
              ...
              <CardContent sx={{ p: 3 }}>
                ...
                >
                  Created by {group.owner.email} • {group.members.length} members
                </Typography>
                      ...
                      <Box sx={{ 
                        ...
                        <Avatar sx={{ 
                          bgcolor: member.isOffline 
                            ...
                            : member.userId === user?.id ? '#3B82F6' : '#6B7280',
                          width: 40,
                          height: 40
                        }}>
                        ...
                        <Box sx={{ flex: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            >
                              {member.isOffline ? member.nickname : member.user?.email}
                ...
                <Box sx={{ 
                  ...
                  {currentUserMember ? (
                    <Button
                      ...
                      onClick={isOwner ? () => setDeleteGroupDialogOpen(true) : handleLeaveGroup}
            ...
            <Box sx={{ py: 2 }}>
              ...
              <Box sx={{ 
                ...
                <Box sx={{ 
                  ...
                  >
                    {group ? `${window.location.origin}/groups/${group.id}` : ''}
                  </Typography>
                  <Button
                    size="small"
      ...
      <Dialog open={editGroupDialogOpen} onClose={() => setEditGroupDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>編輯群組名稱</DialogTitle>
        <DialogContent>
          <TextField
            fullWidth
            label="群組名稱"
